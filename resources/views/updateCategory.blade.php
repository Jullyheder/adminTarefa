        ...
        <x-auth-card>
            ...
            <!-- Description -->
                <div>
                    ...
                    <x-input id="category_desc" class="block mt-1 w-full" type="text" name="category_desc" value="{{ $category->category_desc }}" required autofocus />
                </div>
                ...
                <div class="mt-4">
                    ...
                    <select id="priority_id" name="priority_id" class="form-select" aria-label="Default select example">
                        @foreach($priorities as $priority)
                            <option value="{{ $priority->id }}" {{ $category->priority_id === $priority->id ? 'selected' : '' }}>{{ $priority->priority_desc }}</option>
                        @endforeach
                    </select>
                </div>
